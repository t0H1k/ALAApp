...
class DataStore {
    ...
    let content = [
        [
            "В данном методе можно выполнить большинство настроек для контроллера. Он вызывается ровно один раз, когда контроллер представления загружается в память.Здесь можно инициализировать переменные и объекты, которые должны жить на протяжении всего жизненного цикла вью.На этом этапе границы вью еще не установлены и мы не можем обращаться к значениям геометрии (высоте, ширине и прочим параметрам)",
